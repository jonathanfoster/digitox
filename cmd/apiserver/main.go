...
var (
	...
	signingKeyPath   = app.Flag("signing-key", "RSA private key path for signing JWT tokens.").Default(*dirname + "signing-key.pem").Envar("DIGITOX_SIGNING_KEY").String()
	verifyingKeyPath = app.Flag("verifying-key", "RSA public key path verifying JWT tokens.").Default(*dirname + "verifying-key.pem").Envar("DIGITOX_VERIFYING_KEY").String()
